    ...
    public function getMemories(){
        ...
        $mem_tab = DB::table('tpz_memories as tm')
                    ->select(\DB::raw("tm.sn, tdp.name as file, tdp.datatype, tm.ownerid as userid, tm.entrydate,
                            ...
                            (SELECT tdp2.name FROM tpz_data_deposit as tdp2
                                WHERE tdp2.sn=(SELECT avatar FROM tpz_profile WHERE sn=tm.ownerid))
                            as avartar,
        ...
        for($m=0; $m<count($mem_tabA); $m++){
            ...
            for($r=0; $r<count($results); $r++){
                if($results[$r]["userid"]==$mem_tabA[$m]["userid"]){
                    $new=false;
                    $index=$r;
                    break;
                }
            ...
            if($new){
                ...
                "username"=>$mem_tabA[$m]["firstname"]." ".$mem_tabA[$m]["lastname"],
                "avartar"=>($mem_tabA[$m]["avartar"]!="")?"assets/imagion/{$mem_tabA[$m]["avartar"]}":"",
                //"count"=>$mem_tabA[$m]["engagements"],
                "status"=>  array()
                );

                $results[$m]["status"][]=array(
                    "id"=>$mem_tabA[$m]["sn"],
                    "file"=>($mem_tabA[$m]["file"]!="")?"assets/imagion/{$mem_tabA[$m]["file"]}":"",
                    "caption" =>$mem_tabA[$m]["caption"],
                    "created"=>$mem_tabA[$m]["entrydate"]
                );
            }else{
                $results[$index]["status"][]=array(
                                            "id"=>$mem_tabA[$m]["sn"],
                                            "file"=>"assets/imagion/{$mem_tabA[$m]["file"]}",
                                            "caption" =>$mem_tabA[$m]["caption"],
                                            "created"=>$mem_tabA[$m]["entrydate"]
                                        );
            }
